...
class SyncClient {
  ...
  initSyncDoc = async (workerSid, conferenceSid, supervisorFullName, supervisorStatus, updateStatus) => {
    ...
    if (doc.data.data.supervisors !== null) {
      supervisorList.concat(...doc.data.data.supervisors);
    }
  ...
  /**
   * Returns the Sync Document instance for a temporary doc
   * @param docName the Sync Document to return
   */
  getTempSyncDoc = async (docName) => {
    return this.#client.document({ id: docName, ttl: 1209600 });
  };
  
  /**
   * This is where we update the Sync Document for a temporary doc
   * @param docName the doc name to update
   * @param data the object to update the doc with
   */
  updateTempSyncDoc = async (docName, data) => {
    const doc = await this.getSyncDoc(docName);
