...
export default class SupervisorBargeCoachButton extends AbstractSyncComponent {
   ...
   */
  onDocUpdated = (doc) => {
    const supervisors = doc.data.data.supervisors || [];
  ...
  get supervisorParticipant() {
    ...
    const participant = children.find(
      (p) =>
        p.data.participant_type === 'supervisor' && p.data.status === 'joined' && myWorkerSid === p.data.worker_sid,
    );
  ...
  get agentParticipant() {
    const children = this.props.task?.conference?.source?.children || [];
    const participant = children.find(
      (p) => p.data.participant_type === 'worker' && this.props.agentWorkerSid === p.data.worker_sid,
    );
